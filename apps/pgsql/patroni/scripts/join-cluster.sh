...
# 3) remove PVCs
oc -n $NAMESPACE delete pvc -l "statefulset=${TARGET_STATEFULSET}" --wait
...
oc -n $NAMESPACE patch "statefulset/$TARGET_STATEFULSET" -p "{\"spec\": {\"template\":{\"metadata\":{\"labels\":{\"cluster-name\":\"${PATRONI_SCOPE}\"}}}}}"
...
oc -n $NAMESPACE patch "$(oc -n $NAMESPACE get service -l "cluster-name=${ORIGINAL_PATRONI_SCOPE}" -o name)" -p "{\"spec\":{\"selector\":{\"cluster-name\":\"${PATRONI_SCOPE}\"}}}"
